    ...
    def run(self):        
        ...
        for time_step in tqdm(range(self.args.time_steps)):                                
            ...
            for k in range(self.args.num_drones):
                ...
                data[:, k, image_length + action_length: image_length + action_length + reward_length] = r[k]
            ...
            if len(self.buffer) >= self.args.wait_steps:
                ...
                for agent_id, agent in enumerate(self.agents):         
                    [q, r, next_q] = agent.get_samples(self.buffer[random_index], time_step)                         
                    agent.learn(q, r[:, agent_id], next_q, time_step)                                                                                    
